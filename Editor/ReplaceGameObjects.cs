...
namespace Essentials
{
    ...
    public class ReplaceWithPrefab : EditorWindow
    {
        ...
        public bool SearchWithKeyword = false;
        ...
        void OnGUI()
        {
            ...
            {
                editMode = GUILayout.Toggle(editMode, new GUIContent("Start replacing", "Start using this feature"),
                    EditorStyles.toggle);
            ...
            SearchWithKeyword = GUILayout.Toggle(!SearchWithLayer && !SearchWithTag && SearchWithKeyword,
                "Apply Search By Keyword", EditorStyles.toggle);
            SearchWithTag = GUILayout.Toggle(!SearchWithLayer && !SearchWithKeyword && SearchWithTag,
                "Apply Search By Tag", EditorStyles.toggle);
            SearchWithLayer = GUILayout.Toggle(!SearchWithTag && !SearchWithKeyword && SearchWithLayer,
                "Apply Search By Layer", EditorStyles.toggle);
            if (SearchWithTag)
            {
                GUILayout.Space(5);
            ...
            else if (SearchWithKeyword)
            {
                GUILayout.Space(5);
                KeywordForSearch = EditorGUILayout.TextField("Set keyword :  ", KeywordForSearch);
            }
            ...
            if (editMode)
            {
                ...
                if (prefab != null)
                {
                    ...
                    {
                        GUILayout.Label(
                            new GUIContent("Objects to be Replaced:",
                                !SearchWithTag && !SearchWithLayer && !SearchWithKeyword
                                    ? "Multi-select all the objects you want to replace with your Prefab"
                        ...
                        scrollPosition = GUILayout.BeginScrollView(scrollPosition, EditorStyles.helpBox);
                        {
                            GUILayout.BeginHorizontal(EditorStyles.helpBox);
                            {
                                ...
                                if (replacementPreferences.renameObjects &&
                                    !replacementPreferences.orderHierarchyToPreview)
                                {
                                ...
                                else
                                {
                                    if (replacementPreferences.orderHierarchyToPreview &&
                                        !replacementPreferences.renameObjects)
                                    {
                        ...
                        GUILayout.Space(5);
                        replacementPreferences.renameObjects = GUILayout.Toggle(replacementPreferences.renameObjects,
                            "Rename replaced objects", EditorStyles.toggle);
                        replacementPreferences.orderHierarchyToPreview = GUILayout.Toggle(
                            replacementPreferences.orderHierarchyToPreview, "Oder hierarchy to preview",
                            EditorStyles.toggle);
                        GUILayout.Space(10);
                        replacementPreferences.applyPosition = GUILayout.Toggle(replacementPreferences.applyPosition,
                            "Apply position", EditorStyles.toggle);
                        replacementPreferences.applyRotation = GUILayout.Toggle(replacementPreferences.applyRotation,
                            "Apply rotation", EditorStyles.toggle);
                        replacementPreferences.applyScale = GUILayout.Toggle(replacementPreferences.applyScale,
                            "Apply scale", EditorStyles.toggle);
                    }
                    else
                    {
                        if (!SearchWithTag && !SearchWithLayer && !SearchWithKeyword)
                        {
                            GUILayout.Label(
                    ...
                    if (prefab != null && objectsToReplace.Count > 0)
                    {
                        if (GUILayout.Button("Apply"))
                        {
                            ...
                            EditorSceneManager.MarkSceneDirty(SceneManager
                                .GetActiveScene()); // Important so that we don't forget to save!
                        }
                        else
                        {
        ...
        void SetExistingNumbers(List<GameObject> objects, List<int> ExistingNumbers, NamingScheme namingScheme)
        {
            foreach (var obj in objects)
            {
                ...
                if (name.Contains(prefab.name) && name.Any(char.IsDigit))
                {
                    ...
                    {
                        Debug.LogError(
                            "The selected object cannot be renamed, as there are several naming schemes used");
                    }
                }
            }
        }
        ...
        void Replace(GameObject obj)
        {
            ...
            newObject =
                PrefabUtility.InstantiatePrefab(PrefabUtility.GetCorrespondingObjectFromSource(prefab)) as GameObject;
            ...
            {
                Debug.LogError("Failed removing " + obj.name);
                throw;
            }
        }
            ...
            foreach (var component in components)
            {
                ...
                while (copier == null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            componentType =
                                componentType.BaseType; // Components may be derivatives, so look up the tree.
                        }
                    }
                }
        ...
        void GetSelection()
        {
            ...
            if (editMode && Selection.activeGameObject != null &&
                (prefab == null || (!SearchWithTag && !SearchWithLayer && !SearchWithKeyword)))
            {
            ...
            if (editMode && prefab != null && (SearchWithTag || SearchWithLayer || SearchWithKeyword))
            {
                var currentlyActivePrefab = PrefabStageUtility.GetCurrentPrefabStage()?.prefabContentsRoot;
                var allGameObjects = FindObjectsOfType<GameObject>().Where(gg =>
                {
                    var main = PrefabUtility.GetNearestPrefabInstanceRoot(gg);
                    if (main && main != gg && main != currentlyActivePrefab)
                    {
                        return false;
                    }
                ...
                else if (SearchWithKeyword)
                {
                    ...
                    foreach (var gg in allGameObjects)
                    {
                        if (gg.name.Contains(KeywordForSearch))
                        {
                            if (gg != prefab)
                            {
                                objectsToReplace.Add(gg);
            ...
            else if (editMode && Selection.activeGameObject == null && prefab != null && !SearchWithTag &&
                     !SearchWithLayer && !SearchWithKeyword)
            {
                ResetPreview();
                objectPreview.Clear();
                objectsToReplace.Clear();
        ...
        {
            public void CopyComponent(ReplacementPreferences replacementPreferences, Component original,
                GameObject newObject)
            {
